...
const icons = {
  down: (
    <svg
      ...
      className="fill-current text-[#ffaf23]"
      width="16"
      height="16"
      viewBox="0 0 24 24"
    >
  ...
  cross: (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="fill-current text-[#E02B20]"
      width="16"
      height="16"
      viewBox="0 0 24 24"
    >
...
export default function Form() {
  ...
  return (
    <FormProvider {...methods}>
      <form
        className="w-full xs:w-11/12 md:w-9/12 space-y-16"
        onSubmit={methods.handleSubmit(mergeDocuments)}
      >
        ...
        <div className="mt-10 bg-white rounded-lg mx-auto p-5">
          <Documents />
          {methods.formState.errors.documents && methods.watch("documents").length < 2 && (
            <Error>{methods.formState.errors.documents.message}</Error>
          )}
        </div>
  ...
  return (
    <>
      ...
      <input
        ...
        accept=".pdf"
        onChange={e =>
          e.target.files[0] ? append({ id: uniqueId(), file: e.target.files[0] }) : null
        }
      />
      <label
        htmlFor="pdf"
