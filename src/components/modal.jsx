...
import { Error, Input } from "./form"
import { useFormContext } from "react-hook-form"

export function ConfirmationModal({ isOpen, onClose, onSubmit }) {
          ...
          >
            <div className="w-full inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:p-6">
              <div className="mt-3 text-center">
                <div className="mt-3">
                  <Input onSubmit={onSubmit} onClose={onClose} />
                </div>
              </div>
              {formState.errors.newDocumentName && (
                <Error>{formState.errors.newDocumentName.message}</Error>
              )}
